...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm_rotate(trans, (float)glfwGetTime(), srcVec3);
        vec3Src[0] = 0.5f;
        vec3Src[1] = -0.5f;
        vec3Src[2] = 0.0f;
        glm_vec3_make(vec3Src, srcVec3);
        glm_translate(trans, srcVec3);
...
void processInput(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && mix_strenght < 1.0f)
        mix_strenght += 0.00025f;
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && mix_strenght > 0.0f)
        mix_strenght -= 0.00025f;
}
